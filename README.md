...
- **Demo JWT auth**: each role switch uses a signed demo JWT generated by
  `/api/auth/demo`. The IDs/passwords below are intentionally non-secret and
  exist only to demonstrate role-scoped API access.
...
|---|---|---|---|
| `engineer` | `eng-demo` | engineer | everything except audit |
...
Legacy header tokens (`eng-demo`, `tech-demo`, `comp-demo`, `admin-demo`) still
work for quick curl/API tests, but the UI uses demo JWTs.

---

## Benchmarks (`python -m chronos.eval.benchmark`)

Measured on the bundled synthetic plant. The main table is **on controlled
synthetic validation**, not a claim of production accuracy:
...
CHRONOS also reports a **noisy validation** track with false alarms, incomplete
work orders, missing tags, typo-filled records, and healthy assets with similar
vibration spikes. Those scores are intentionally imperfect and are meant to show
reasonable behavior under plant-data noise rather than a polished happy path.

The Accuracy tab also shows demo business-impact estimates:

| Impact lever | Demo value |
|---|---|
| Pump trip cost | INR 450,000/hour or $5,400/hour |
| Average downtime avoided | 7.5 hours |
| Inspection search time | 3 hours -> 18 seconds |

> Replace these with site-specific loss-of-production, maintenance-hour, and
> downtime assumptions for a real deployment.
...
The generator also writes `var/warehouse/dirty_records.csv` with explicit
counterexamples and dirty plant records: false alarms, incomplete work orders,
missing tags, typo-filled rows, and healthy vibration spikes. The Equipment tab
includes a **Bring Your Own Plant Data** CSV preview that maps a small uploaded
sample into the normalized Event schema and flags rows needing review.

Confidence is explainable in the UI. Risk and copilot answers show why a score
such as 92% was assigned: matched failure stages, support cases, recency of the
current stage, and asset criticality, plus source-match evidence for copilot
answers.
